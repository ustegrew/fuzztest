...
public abstract class VBrowseable
{
    ...
    private void _Register0 (String key) /* [130] */
    {
        String      k;
...
[130]   Initially we had a method with two parameters:
            private void _Register (String key, boolean doAutoKey)
        And the two calling methods:
            protected void _Register ()             {_Register (null, true );}
            protected void _Register (String key)   {_Register (key,  false);}
        Unfortunately the code generated by JSweet would not properly call the _Register (key, doAutoKey)
        method. When we did
            _Register ("foo"), 
        expecting it to call
            _Register ("foo", false)
        the generated method would call instead
            _Register (null, true)
            
        The present arrangement will call _Register0 (...) correctly. And, apart from that
        the code now makes more sense anyway - why do we need an extra boolean variable anyway
        when we can simply pass null as key and test for null in the _Register0 method?
*/
